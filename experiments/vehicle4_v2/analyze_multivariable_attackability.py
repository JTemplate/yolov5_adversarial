...
def make_stacked(frame: pd.DataFrame) -> pd.DataFrame:
    learned = frame.copy()
    learned["kind"], learned["successes"], learned["trials"] = (
        1.0,
        learned["learned_attacked_count"],
        learned["condition_pairs"],
    )
    random = frame.copy()
    random["kind"], random["successes"], random["trials"] = (
        0.0,
        random["random_attacked_count"],
        random["condition_pairs"],
    )
            ...
            rows.append(
                {
                    ...
                    "factor_label": FACTOR_LABELS[factor],
                    "level": level,
                    "adjusted_attack_probability": pred,
                }
            )
    return rows
...
def main() -> None:
    ...
    for outcome in ["learned_attacked_count", "random_attacked_count"]:
        ...
        coeffs.extend(full["rows"])
        importances.extend(rows)
        effects.extend(conditional_effects(full, outcome))
    _stacked, paired_fit, paired_rows = paired_importance(df, args.folds)
    coeffs.extend(paired_fit["rows"])
    importances.extend(paired_rows)

    pd.DataFrame(coeffs).to_csv(args.output / "coefficients.csv", index=False)
    pd.DataFrame(importances).sort_values(["outcome", "partial_deviance"], ascending=[True, False]).to_csv(
    ...
    manifest = {
        ...
        "outcomes": {
            "learned_attacked_count": "learned patch attack",
            "random_attacked_count": "random patch attack",
            "paired": "stacked learned/random model with kind interactions",
        },
        "covariates": {
            "class": "categorical",
            "clean_confidence": "six fixed buckets",
            "bbox_area": "six fixed pixel-area buckets",
            "image_density": "five fixed detections-per-image buckets",
            "model": "categorical detector fixed effect",
        },
        ...
        "caveats": [
            "objects are detector clean predictions, not ground-truth objects",
            "bbox area and density are partly scene/model dependent",
            "additive model is associational; it is not a causal intervention",
            "size is omitted from the main model because it is a deterministic coarsening of bbox area",
        ],
    }
    ...
    lines = [
        ...
        "",
        "The estimates are conditional associations. They do not prove that changing confidence, area, or density would cause a change in ASR, because these variables are properties of the detector/image and are correlated.",
        "",
    ]
    for outcome, label in [
        ...
        imp = pd.DataFrame([r for r in importances if r["outcome"] == outcome]).sort_values(
