...
def parse_args():
    ...
    parser.add_argument(
        "--analysis-root", type=Path, default=ROOT / "experiment_data/vehicle4_v2/analysis/method_comparison_v1"
    )
...
def train_wave(args, template: dict, method: str, heartbeat: Path) -> None:
    ...
    for index, (seed, physical_gpu) in enumerate(zip(SEEDS, PHYSICAL_DEVICES)):
        ...
        command = [
            sys.executable,
            str(ROOT / "experiments/vehicle4_v2/train_patch_variant.py"),
            "--cfg",
            str(config_path),
            "--variant",
            method,
...
def write_registry(args, method: str) -> Path:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps({"method": method, "source_model": "yolov5s_vehicle4_v2_seed0", "patches": records}, indent=2)
        + "\n",
        encoding="utf-8",
    )
    return path
...
def run_logged(args, method: str, stage: str, command: list[str], env: dict | None = None) -> None:
    ...
    if result.returncode:
        raise RuntimeError(
            f"{method} {stage} failed with exit code {result.returncode}; see {log_dir / (stage + '.log')}"
        )
...
def main():
    ...
    write_heartbeat(
        heartbeat,
        {"stage": "starting", "updated_at": time.time(), "methods": METHODS, "seeds": SEEDS, "weights": str(weights)},
    )
    for method in METHODS:
        ...
        if not args.skip_evaluation:
            ...
            render_env["CUDA_VISIBLE_DEVICES"] = "3"
            render_command = [
                sys.executable,
                str(ROOT / "experiments/vehicle4_v2/render_attacks.py"),
                "--patch-registry",
                str(registry),
                "--output-root",
                str(args.run_root / method / "rendered"),
                "--device",
                "cuda:0",
            ]
            ...
            eval_command = [
                sys.executable,
                str(ROOT / "experiments/vehicle4_v2/evaluate_shared.py"),
                "--model",
                "yolov5s",
                "--weights",
                str(weights),
                ...
                str(args.run_root / method / "rendered"),
                "--output-root",
                str(args.run_root / method / "evaluation"),
                "--device",
                "cuda:0",
                "--batch-size",
                "8",
    ...
    if not args.skip_evaluation:
        command = [
            sys.executable,
            str(ROOT / "experiments/vehicle4_v2/summarize_hard_target_methods.py"),
            "--run-root",
            str(args.run_root),
            "--analysis-root",
        ...
        if result.returncode:
            raise RuntimeError("method summary failed; see summary.log")
